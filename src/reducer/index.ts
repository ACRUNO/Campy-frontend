import {
  ...
  GET_ALLCAMPINGS,
  GET_LOCALIDADES,
  GET_CAMPINGS_PROVINCIAS,
  GET_CAMPINGS_LOCALIDADES,
  GET_DETAILS,
  FILTER_PROVINCIA,
  FILTER_LOCALIDAD,
  CREATE_CAMPING,
  ...
  POP_UP_CARD,
  SET_CARD_INFO,
  FILTER_PROVINCIA_MAP,
  FILTER_LOCALIDAD_MAP,
  FILTER_INGRESO_MAP,
  FILTER_EGRESO_MAP,
  ZOOM_OUT_MAP,
  CLEAN_DETAILS,
  GET_ALL_LOCALIDADES,
  RESET_FILTER_CAMPING,
  DIAS_RESERVADOS_BOOKING,
} from "../actions";
import { LOGIN_USER, LOGOUT_USER } from "../actions/Login.action";
import {
  GET_FAVORITES_CAMPINGS,
  GET_OWNER_CAMPINGS,
  GET_USER_BOOKINGS,
...
import {
  ...
  GET_USUARIOS_BYNAME,
  GET_CAMPINGS_BYNAME,
  GET_RESERVAS_CAMPING,
  CLEAN_RESERVAS_CAMPING,
} from "../actions/Dash.admin.action";
  ...
  switch (action.type) {
    ...
    case FILTROS_COMBINADOS:
      ...
      if (!filtrosBook.includes(action.payload.value)) {
        ...
        filtrosBook = filtrosBook.filter(
      ...
      return {
        ...
        filtrosBooking: {
          ...
          [action.payload.name]: filtrosBook,
        },
      };
    ...
    case FILTER_INGRESO_MAP: {
      return {
        ...
        filtrosBooking: {
          ...state.filtrosBooking,
          abierto_fecha_desde: action.payload
            ?.toDate()
            .toLocaleDateString()
            .split("/")
            .reverse()
    ...
    case FILTER_EGRESO_MAP:
      return {
        ...
        filtrosBooking: {
          ...state.filtrosBooking,
          abierto_fecha_hasta: action.payload
            ?.toDate()
            .toLocaleDateString()
            .split("/")
            .reverse()
    ...
    case ZOOM_OUT_MAP:
      return {
        ...state,
        LinkMap: {
          lat: -38.40725346022871,
          lng: -63.617129400239264,
          zoom: 10,
        },
      };
    ...
    case REMOVE_FAVORITE_CAMPING:
      return {
        ...state,
        favoritesCampings: {
          favorites: state.favoritesCampings.favorites.filter(
            (fav: { id: number }) => fav.id !== action.payload
          ),
          done: true,
        },
      };
    ...
    case GET_USUARIOSCAMPY:
      ...
      while (
        ...
        if (new Date(action.payload[i].createdAt).valueOf() < day.valueOf()) {
          ...
          let obj: { users: number; created: string } = {
            users: u,
            created: day.toLocaleDateString("zh-Hans-CN"),
          };
          data.push(obj);
    ...
    case GET_RESERVASCAMPY:
      ...
      let ordenado = action.payload.sort(
        (
          ...
          b: { createdAt: string; total: number }
        ) =>
      ...
      let j: number = 0;
      while (
        dia.toISOString().valueOf() < new Date().toISOString().valueOf() &&
        j < ordenado.length
      ) {
        if (new Date(ordenado[j].createdAt).valueOf() < dia.valueOf()) {
          ...
          let obj: { reservas: number; total: number; created: string } = {
            reservas: r,
            total: t,
            created: dia.toLocaleDateString("zh-Hans-CN"),
          };
          datos.push(obj);
          //sumo 7 dias
    ...
    case GET_USUARIOS_BYNAME:
      if (action.payload.length > 0) {
        var usuariosBuscados: {
          id: number;
          username: string;
          email: string;
          tipo: string;
          habilitado: number;
        }[] = state.usuariosDash_All.filter(
          (u: {
            id: number;
            username: string;
            email: string;
            tipo: string;
            habilitado: number;
          }) => u.username.toLowerCase().includes(action.payload.toLowerCase())
        );
      } else {
        var usuariosBuscados: {
          id: number;
    ...
    case LINK_MAP:
      return {
        ...state,
        linkMap: {
          lng: Number(action.payload.longitud),
          lat: Number(action.payload.latitud),
          zoom: 16,
        },
      };
    case GET_CAMPINGS_BYNAME:
      if (action.payload.length > 0) {
        var campingsBuscados: {
          id: number;
          nombre_camping: string;
          habilitado: number;
          localidad: string;
          provincia: string;
        }[] = state.campingsDash_All.filter(
          (u: {
            id: number;
            nombre_camping: string;
            habilitado: number;
    ...
    case DISABLE_OWNER_CAMPING:
      const campings = state.ownerCampings.campings.map((camping: any) => {
    ...
    case GET_ALLPOSTS:
      ...
      let postsvis = action.payload
        .slice()
        .sort((a: allPosts, b: allPosts) =>
      ...
      if (action.payload.length > 0) {
        var postsBuscados: {
          ...
          (p: {
            ...
            cant_comentarios: number;
            cant_visualizaciones: number;
          }) => p.titulo.toLowerCase().includes(action.payload.toLowerCase())
        );
      } else {
        var postsBuscados: {
          titulo: string;
